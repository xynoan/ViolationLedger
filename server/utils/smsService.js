...
export async function NoPlateNumberSms(locationId, violationId) {
  try {
    // Get all vehicles with empty or NULL plate numbers
    const vehicles = db
      .prepare('SELECT * FROM vehicles WHERE plateNumber IS NULL OR plateNumber = ""')
      .all();

    if (!vehicles.length) {
      console.log('No vehicles with empty plate numbers found.');
      return { success: true, message: 'No vehicles to notify enforcers.' };
    }

    const enforcers = db.prepare('SELECT * FROM barangay_user WHERE contactNumber IS NOT NULL AND contactNumber != ""').all();

    if (!enforcers.length) {
      console.error('No enforcers found with valid contact numbers.');
      return { success: false, error: 'No enforcers to send SMS.' };
    }
    ...
    for (const vehicle of vehicles) {
      const message =
        `Hi, an illegally parked vehicle has been detected at ${locationId} on ${currentTime}. ` +
        `Kindly respond and take necessary action to resolve the situation.`;

      for (const enforcer of enforcers) {
        const smsResult = await sendSmsMessage(enforcer.contactNumber, message);

        const messageLogId = `SMS-${violationId}-${vehicle.id}-${Date.now()}`;
        const sentAt = new Date().toISOString();
        ...
        console.log(`SMS sent to enforcer ${enforcer.contactNumber} for vehicle ID ${vehicle.id}: ${smsResult.success ? 'Success' : 'Failed'}`);
      }
    }

    return { success: true, message: 'SMS notifications sent to all enforcers for vehicles without plates.' };

  } catch (err) {
    console.error('[SMS] NoPlateNumberSms exception:', {
      locationId,
      violationId,
      error: err.message,
      stack: err.stack
    });
    return { success: false, error: err.message || 'Unexpected error in NoPlateNumberSms' };
  }
}
